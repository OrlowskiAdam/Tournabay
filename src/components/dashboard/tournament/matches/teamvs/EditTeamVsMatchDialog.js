...
const EditTeamVsMatchDialog = (props) => {
  const { tournament } = useTournament();
  const { closeModal, match } = props;
  const [redTeam, setRedTeam] = useState(match.redTeam);
  const [blueTeam, setBlueTeam] = useState(match.blueTeam);
...
EditTeamVsMatchDialog.propTypes = {
  closeModal: PropTypes.func.isRequired,
  match: PropTypes.object.isRequired,
};

export default EditTeamVsMatchDialog;
