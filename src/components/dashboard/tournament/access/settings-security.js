...
const SettingsSecurity = () => {
  ...
  const [rolesValue, setRolesValue] = useState(
    tournament.permission.canTournamentRoleManageSettings
  );
  const [staffMembersValue, setStaffMembersValue] = useState(
    tournament.permission.canStaffMemberManageSettings
  );
  ...
  const handleSaveClick = () => {
    setIsLoading(true);
    const toastLoadingId = toast.loading("Updating settings permissions.");
    tournamentPermissionApi
      .updateSettingsPermission(tournament.id, {
        tournamentRoles: rolesValue,
        staffMembers: staffMembersValue,
      })
      .then((response) => {
        dispatch(updatePermission(response.data));
        toast.success("Settings permissions updated successfully.");
      })
      .catch((error) => {
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column" }}>
      <h3>Settings</h3>
      <p>Can enter the settings page and manage them:</p>
      <Box>
